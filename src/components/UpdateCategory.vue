<template>
  ...
  <div class="d-flex justify-content-center align-center mt-4">
    <v-card title="Add New Catego" width="500">
      <v-card-text>
        <v-sheet>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn text="Add Now" @click="updateCatego()" color="green"></v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getCategoryName(userId, locId, catId) {
      let result = await axios.get(
        `http://localhost:3000/Catego?userId=${userId}&locationId=${locId}&id=${catId}`
      );
      if (result.status == 200) {
    ...
    async updateCatego() {
      ...
      if (!this.v$.$error) {
        if (filterCategoryName.length > 0) {
          this.userNotFound = "The Category Name Is Taken";
          this.successMessage = "ُ";
        } else {
          let result = await axios.put(
            `http://localhost:3000/Catego/${this.catId}`,
            {
              name: this.name,
              userId: this.userId,
          ...
          if (result.status == 200) {
            this.userNotFound = "";
            this.successMessage = "Updated Category Name Is Successfully";
            setTimeout(() => {
              this.$router.push({
                name: "ViewCategories",
